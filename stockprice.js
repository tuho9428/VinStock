...
app.get("/stockprice", (req, res) => {
  res.render("stockprice.ejs", { content: "" , symbol: ""});
});
...
app.post("/stockprice", async (req, res) => {
  ...
  try {
    ...
    res.render("stockprice.ejs", { content: prices, symbol: stockSymbol });
  } catch (error) {
    console.error(error);
    res.render("stockprice.ejs", { content: "Error!", symbol: symbol });
  }
});
